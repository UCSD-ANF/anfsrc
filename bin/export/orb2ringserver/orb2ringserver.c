...
main (int argc, char **argv)
{
  ...
  if ( parameter_proc (argc, argv) < 0 )
    {
      ms_log (1, "Argument processing failed\n");
      ms_log (1, "Try '-h' for detailed help\n");
      return -1;
    }
  ...
  if ( verbose )
    ms_log (1, "Connected to orbserver at %s\n", orbaddr);
  
  /* Init the ORB position */
  if ( startat )
    {
      if ( verbose )
	ms_log (1, "Initializing ORB position to '%s'\n", startat);
      ...
      if ( isalldig (startat) )  /* if all digits assume pktid */
	{
	  if ( orbseek (orb, atoi(startat)) == -1 )
	    ...
	    ms_log (1, "ORB positioned to pktid: %d\n", atoi(startat));
	}
      else if ( is_epoch_string (startat, &tepoch) )
	{
	  orbposition (orb, startat);
	  if ( verbose )
	    ms_log (1, "ORB positioned to time: %s\n", startat);
	}
    }
  ...
  if ( statefile )
    {
      if ( verbose )
	ms_log (1, "Resuming ORB position from state file\n");
	...
	{
	  ms_log (1, "ORB position resurrection unsuccessful\n");
	}
    }
  ...
  while ( ! stopsig )
	...
	{
	  ms_log (1, "orbreap() error (%d)\n", orbret);
	  stopsig = 1;
	}
      else if ( orbret != ORB_INCOMPLETE ) /* Process the packet */
	{
	  if ( verbose >= 3 )
	    ms_log (1, "Recv'd %s, pktid %d\n", srcname, pktid);
	  ...
	  if ( statefile && stateinter )
	      ...
	      if ( pktcount >= stateinter )
		{
		  ...
		  if ( (bury()) < 0 )
		    ...
		    ms_log (1, "ORB position saved to state file\n");
		}
	    }
  ...
  if ( statefile ) 
    {
      if ( (bury()) < 0 )
	ms_log (1, "error saving ORB position to state file\n");
      else if ( verbose )
	ms_log (1, "ORB position saved to state file\n");
    }
	      ...
	      char *rawpacket, int nbytes)
{
  ...
  msr->starttime = MS_EPOCH2HPTIME(pktchan->time);
  ...
  if ( origreclen == 512 )
	...
	{
	  ms_log (3, "Cannot add packet data to trace buffer!\n");
	  return -1;
	}
...
parameter_proc (int argcount, char **argvec)
{
  ...
  if ( errflag )
    {
      ms_log (1, "Usage: %s [options] ORB ringserver\n", PACKAGE);
      ms_log (1, "Try '-h' for detailed help\n");
      exit (1);
    }
