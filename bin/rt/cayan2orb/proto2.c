...
void p2_start2orb(int orbfd, unsigned char *buf)
{
  ...
  *(orbpkt->parts.src_loc)=0;
  ...
  p2_numframes(orbpkt, epochstr, buf);
  p2_voltage(orbpkt, epochstr, buf);
  p2_firmware(orbpkt, epochstr, buf);
  p2_mmcstat(orbpkt, epochstr, buf);
...
void p2_data2orb(int orbfd, unsigned char *buf)
{
  ...
  *(orbpkt->parts.src_loc)=0;
  ...
  p2_pressure(orbpkt, epochstr, buf);
  p2_nwind0(orbpkt, epochstr, buf);
  p2_ewind0(orbpkt, epochstr, buf);
  p2_ngust0(orbpkt, epochstr, buf);
  p2_egust0(orbpkt, epochstr, buf);
  p2_nwind1(orbpkt, epochstr, buf);
  p2_ewind1(orbpkt, epochstr, buf);
  p2_ngust1(orbpkt, epochstr, buf);
  p2_egust1(orbpkt, epochstr, buf);
  p2_temp0(orbpkt, epochstr, buf);
  p2_temp1(orbpkt, epochstr, buf);
  p2_humidity(orbpkt, epochstr, buf);
  p2_rain(orbpkt, epochstr, buf);
  p2_solar(orbpkt, epochstr, buf);
...
void p2_firmware(struct Packet *orbpkt, char *staid, unsigned char *buf)
{
  ...
  /* firmware */
  if (verbose)
    fprintf(stderr,"adding channel firmware version\n");
  pktchan = newPktChannel();
  pktchan -> datasz = 1;
  pktchan->data=malloc(4);
  if (pktchan->data==NULL)
  ...
  pktchan->data[0]=buf[19]; 
  pktchan->time=orbpkt->time;
  strncpy(pktchan->net,NETNAME,2);
  strncpy(pktchan->sta,staid,5);
  strncpy(pktchan->chan,"firmV",5);
  *(pktchan->loc)='\0';
  strncpy(pktchan->segtype,"c",4);
  pktchan->nsamp=1;
  pktchan->calib=0;
...
void p2_mmcstat(struct Packet *orbpkt, char *staid, unsigned char *buf)
{
  ...
  /* MMC Status */
  if (verbose)
    fprintf(stderr,"adding channel MMC Status\n");
  pktchan = newPktChannel();
  pktchan -> datasz = 1;
  pktchan->data=malloc(4);
  if (pktchan->data==NULL)
  ...
  pktchan->data[0]=buf[20]; 
  pktchan->time=orbpkt->time;
  strncpy(pktchan->net,NETNAME,2);
  strncpy(pktchan->sta,staid,5);
  strncpy(pktchan->chan,"MMCstat",7);
  *(pktchan->loc)='\0';
  strncpy(pktchan->segtype,"c",4);
  pktchan->nsamp=1;
  pktchan->calib=0;
