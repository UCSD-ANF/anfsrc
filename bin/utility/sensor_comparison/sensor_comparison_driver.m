...
ERROR_LOG = fopen(sprintf('/anf/%s/work/white/xcal/%s/error_log',NETWORK,datestr(now,'yyyymmdd')),'a');
...
for i=1:length(event_dbs)
    ...
    if dbnrecs(db_events) > 0
        events = dbgetv(db_events,'prefor');
        dbclose(db);
        ...
        for j=1:length(events)
            ...
            db_origin = dbsubset(db_origin, sprintf('orid == %s',prefor));
            ...
            for k=1:length(proximal_stas)
                ...
                if has_colocated_sensors(db_sta)
                    ...
                    tic;
                    sensor_comparison(dbpath, NETWORK, prefor, station, [-ptt+tb(1) ptt+tb(2)], strong_chan, weak_chan);
                    toc;
                else
                    dbclose(db);
                end
