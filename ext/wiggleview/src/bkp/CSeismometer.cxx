...
CSeismometer::CSeismometer(string configFile, int stationNum, SbBool g_drawOnGlobe)
{
...
//	m_trans->translation.setValue(longitude, 5,-latitude);
	...
	//float sphereX = sphereRadius * sin(DTOR(longitude))*sin(DTOR(90-latitude));
	//float sphereY = sphereRadius * cos(DTOR(90-latitude));
	//float sphereZ = sphereRadius * cos(DTOR(longitude))*sin(DTOR(90-latitude));
	//m_trans->translation.setValue(sphereX, sphereY, sphereZ);
	//printf("Station %s longitude %f latitude %f\n",bhe->getStationName().c_str(),longitude, latitude);
	...
	//m_rot->rotation.setValue(anAxis,angle);
	...
	sMat->transparency.setValue(0.3f);
...
void CSeismometer::update(float curTime, SbBool drawOnGlobe, int numOfPtsAdded)
{
	if (drawOnGlobe)
	{
		m_trans->translation.setValue(m_sphereTrans[0], m_sphereTrans[1], m_sphereTrans[2]);
