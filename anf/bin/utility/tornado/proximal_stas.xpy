...
def main():
        ...
        #open site table
        site_tbl = datascope.dbopen(sys.argv[1],"r").lookup(table="site").sort("sta",unique=True).sort("ondate")


        #open tortrk table
        tortrk_tbl = datascope.dbopen(sys.argv[2],"r").lookup(table="tortrk").sort("time")
        tortrk_tbl.record = 0
        ...
        for yr in range(first_yr,last_yr+1,1): wfdisc_in[yr] = datascope.dbopen(path2wfdisc + "_" + str(yr), "r").lookup(table="wfdisc")

        path2wfs = sys.argv[4]

        dbout_path = sys.argv[5]
        if not os.path.isfile(dbout_path):
                datascope.dbcreate(dbout_path, "css3.0", dbpath=dbout_path)
                print ("css3.0 database descriptor file created:", dbout_path)

        dbout = datascope.dbopen(dbout_path,"r+")
                        ...
                        if min_dist(site_lat, site_lon, tor_slat, tor_slon, tor_elat, tor_elon) < MAX_DIST and int(site_tbl.getv("ondate")[0]) < tor_YYYYJJJ and int(site_tbl.getv("offdate")[0]) > tor_YYYYJJJ:
                                ...
                                while True:
                                        ...
                                        if wfdisc_in_tmp.record >= 0:
                                                ...
                                                for wfdisc_in_tmp.field in range(wfdisc_in_tmp.query(datascope.dbFIELD_COUNT)):
                                                        field = wfdisc_in_tmp.query(datascope.dbFIELD_NAME)
