...
usage = '\nUSAGE:\n\t%s [-v] db start end sta_regex chan_regex\n\n' % __file__
...
""" 
Get data for our subset of stations 
and channels.
"""
done = {}
trace = 0
for i in range(traces):
    ...
    if rotate:
        pylab.plot(timelist,datalist,'#FFFE00',lw=0.2)
    else:
    ...
    if show_arrivals:
        ...
        for p in range(temp_arrival.query('dbRECORD_COUNT')):
            ...
            if rotate:
                pylab.plot([distance-(0.5*amp_mult),distance+(0.5*amp_mult)],[at,at],'r',lw=0.4)
            else:
...
" Add the distance to the top of the x axis."
if rotate:
    if event_location:
        ax2 = ax.twiny()
        #ax2.set_xlim([x1,x2])
    ...
    y_ax = [stock.epoch2str(y,'%D\n%H:%M UTC') for y in ax.yaxis.get_ticklocs()]
    ax.set_yticklabels(y_ax,fontsize=10)
...
#pylab.text(distance, start, '%s_%s' % (s,c), bbox={'facecolor':'red', 'alpha':0.9, 'pad':1}, fontsize=5, rotation=-90)

#{{{ # Add the line of the wavefront using the provided speed
for s in speed:
    ...
    if rotate:
        " Great Circle. "
        pylab.plot([x1,x2],[(x1/ns)+event_time,(x2/ns)+event_time],'k',lw=1)
        pylab.text(x2, ((x2/ns)+event_time), ' %s m/s' % s, color='r', horizontalalignment='right', verticalalignment='center',
                                bbox={'facecolor':'k', 'alpha':1.0, 'pad':2}, fontsize=10)

        " Great Circle Complement. "
        c_y1 = ((360-x1)/ns)+event_time
        c_y2 = ((360-x2)/ns)+event_time
        pylab.plot([x1,x2],[c_y1,c_y2],'g',lw=1)
        pylab.text(x2, c_y2, ' %s m/s' % s, color='r', horizontalalignment='right', verticalalignment='center',
                                bbox={'facecolor':'g', 'alpha':1.0, 'pad':2}, fontsize=10)
    else:
        " Great Circle. "
        pylab.plot([(y1/ns)+event_time,(y2/ns)+event_time],[y1,y2],'k',lw=1)
        pylab.text(((y2/ns)+event_time), y2, ' %s m/s' % s, color='r', horizontalalignment='right', verticalalignment='center',
                                bbox={'facecolor':'k', 'alpha':1.0, 'pad':2}, fontsize=10)
        ...
        pylab.text(c_x2, y2, ' %s m/s' % s, color='r', horizontalalignment='right', verticalalignment='center',
                                bbox={'facecolor':'g', 'alpha':1.0, 'pad':2}, fontsize=10)
...
pylab.savefig('dataplot.png',bbox_inches='tight', facecolor=pl.get_facecolor(), edgecolor='none',pad_inches=0.5,dpi=200)
