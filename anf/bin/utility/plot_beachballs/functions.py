...
class Origin():
    ...
    def params_subset(self, options):
        ...
        self.params(options)
        express = ' '.join(["lat >=", self.bounds[2], "&&", "lat <=", self.bounds[3],
                "&&", "lon >=", self.bounds[0], "&&", "lon <=", self.bounds[1], "&&",
    ...
    def moment_array(self):
        ...
        if lens == 0:
            ...
            for x in range(0, lens):
                self.table.record = x
                result = self.table.getv("orid", "evid", "mt.auth", "prefor",
                        "lat", "lon", "depth", "drmag", "scm", "tmtt", "tmpp",
                        "tmrr", "tmtp", "tmrt", "tmrp", "time", "estatus",  "str1",
                        "str2", "dip1", "dip2", "rake1", "rake2" )
