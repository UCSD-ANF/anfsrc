...
%-----------------------------------------------------
%
function waveformplots( ev_type, starttime, endtime, ev_database, ev_clustername, wf_database, wf_clustername )
    ...
    %staregex = ['sta=~/' station '/'] ;
    %chanregex = ['chan=~/' strjoin(channels,'|') '/'] ;
    ...
    for c = 1:length(dbObj.databases)

        database = dbObj.databases{c} ;
        ...
        fprintf( 'Waveformplot: %s\n', database ) ;

        db0 = dbopen( database, 'r' ) ;


        fprintf( 'dbsubset %s.wfdisc "%s && %s" |dbselect -\n', database, ['sta=~/' station '/'], ['chan=~/' strjoin(channels,'|') '/']) ;

        %--- Open the wfdisc
        db1 = dblookup_table( db0,'wfdisc' ) ;
        db2 = dbsubset( db1, ['sta=~/' station '/ && chan=~/' strjoin(channels,'|') '/']) ;
        wfrecords = dbquery( db2, 'dbRECORD_COUNT' ) ;
        ...
        if wfrecords < 3
            ...
            try
                ...
                if( strcmp( ev_type, 'regional' ) )
                    mytitle = { [ 'Three components recorded from a regional event detected by station TA ' station ]  } ;
                else
                    mytitle = { [ 'Three components recorded from a large event detected by station TA ' station ]  } ;
                end
                ...
                disp(exception) ;
                fprintf( 'Cannot load trace object data\n' ) ;
                continue ;
