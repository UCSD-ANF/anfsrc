...
def chan_thread(chan, sta, myrrdpath, stadb, null_run):
    ...
    with Timer() as thread_timer:
        ...
        with freeing(stadb.subset('chan =~ /%s/' % chan)) as tempdb:
            ...
            #verbose mode logging
            if verbose:
                main_logger.info(' %s records in database' % tempdb.record_count)
            ...
            for record in tempdb.iter_record():
                ...
                starttime, endtime, nsamp = record.getv('time', 'endtime', 'nsamp')
                #if there are no samples in wfdisc row, log it, and move to
                #next row
                if nsamp == 0:
                    main_logger.info(' no elements in wfdisc record %s %s:%s %s - %s' \
                        % (db, sta, chan, epoch2str(starttime, '%Y%j %T'), \
                        epoch2str(endtime, '%Y%j %T')))
                    continue
                #verbose mode logging
                if verbose:
                    main_logger.info(' %s %s [%s samples]' % (starttime,endtime,nsamp))
                ...
                try:
                    ...
                    raise(Exception('chan_thread: rrdtool.info(rrd) - %s' \
                        % e))
                #make sure that the start time of the data segment to be
                #requested is later than the RRD last update time and skip the
                #segment if the data RRD has already been updated beyond the
                #end of the segment
                ...
                #move on to the next wfdisc row if data can't be retrieved
                try:
                    subset_list, endtime = get_data(record, sta, chan, starttime, \
                        endtime, rrd_max_recs, verbose)
                except Exception as e:
                    main_logger.error('chan_thread():%s\t- skipping %s:%s' % (e, sta, chan))
                    continue
                ...
                if not null_run:
                    for subset in subset_list:
                        try:
                            ...
                            main_logger.error('%s - skipping %s:%s %d - %d (%d)' \
                                % (e, sta, chan, starttime, endtime, starttime_save))
                            continue
                        #verbose mode logging
                        if verbose:
                            main_logger.info(' rrdtool update %s_%s.rrd [%d] recs' \
                                % (sta, chan, len(subset)))
    if nrecs > 0:
...
else: logfile = '/anf/TA/work/white/update_rrd_from_db_%s' % epoch2str(now(), '%Y%j%H%M%S')
...
if args.cluster_name:
    ...
    if os.path.exists('%s.clusters' % dbcentral):
        sys.exit('%s looks like a dbcentral cluster, specify \'-c\' option. quitting.' % dbcentral)
...
for net in sorted(stations.keys()):
    ...
    for sta in sorted(stations[net].keys()):
        with Timer() as sta_timer:
            ...
            #retain only channels matching chan_subset in list of channels
            #to process
            if chan_subset:
            ...
            for dbtime in sorted(dbcentral_dbs.keys()):
                with Timer() as db_timer:
                    ...
                    with closing(dbopen(db, 'r')) as stadb:
                        ...
                        #if there are no matching rows, continue with next database
                        if stadb.record_count == 0:
                            continue
                        #check RRD for each channel at station if not already
                        #checked
                        for chan in pf['Q330_SOH_CHANNELS']:
                            if not rrd_checked[chan]:
                                #build the path to the RRD directory
                                rrd = myrrdpath+'/'+sta
                                #create the station directory to house the RRDs if
                                #necessary
                                if not os.path.exists(rrd) and not null_run:
                                    main_logger.info(' make RRD directory %s ' % rrd)
                                    try:
                                        os.mkdir(rrd)
                                    except Exception as e:
                                        e = ' %s - os.mkdir(rrd)' % e
                                ...
                                if verbose and rebuild:
                                    main_logger.info(' clean RRD table for %s:%s %s' \
                                        % (sta, chan, rrd))
                                #check that rrd exists and if specified, create \
                                #a new, empty RRD
                                try:
                                    check_rrd(rrd, chan, stadb, verbose, rebuild, \
                                        rrd_npts, null_run)
                                except Exception as e:
                                    e = '%s' % e
                                    raise(Exception(e))
                                rrd_checked[chan] = True
                        #loop over all channels for station and create a new thread
                        #for each. If there are MAX_THREAD active channel threads,
                        #sleep for a minute then try again. +1 is for main thread.
                        for chan in pf['Q330_SOH_CHANNELS']:
                            while not threading.active_count() < MAX_THREADS+1:
                                main_logger.info(' active thread count: %d\tsleeping...' \
                                    % threading.active_count()-1)
                                time.sleep(60)
                            #keep trying until a new thread is successfully created
                            successful = False
                            while not successful:
                                try:
                                    #create new thread
                                    new_thread = threading.Thread(target=chan_thread, \
                                        args=(chan, sta, myrrdpath, stadb, null_run))
                                    #start new thread
                                    new_thread.start()
                                    successful = True
                                    main_logger.info(' new thread started for %s:%s\tactive ' \
                                        'thread count: %d' \
                                        % (sta, chan, threading.active_count()-1))
                                except Exception as e:
                                    main_logger.error(e)
                        #make sure all threads reading from this database are
                        #finished before closing database
                        while threading.active_count() > 1:
                            main_logger.info(' station %s still has %d channel threads '\
                                'actively accessing db %s\twaiting to close db...' \
                                 % (sta, threading.active_count()-1, db))
                            time.sleep(60)
                        main_logger.info(' station %s RRD writes complete for db %s/%s' \
                            % (sta, dbcentral_dbs[dbtime]['dir'], \
