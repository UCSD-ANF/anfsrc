...
FLTSZ = sys.getsizeof(float()) / (1024.0 ** 3)
# INTSZ = sys.getsizeof(np.int32()) / (1024. ** 3)
INTSZ = 4 / (1024.0 ** 3)


class SegDException(Exception):
    """Exception for the SEGD class."""

    pass


class SegD:
    """The SegD class for SEISPY."""
                                ...
                                [
                                    ...
                                    (
                                        "extended_header_blocks",
                                        {
                                            ...
                                            "nibbles": 4,
                                            "type": "binary",
                                            "description": "Extended header " "blocks",
                                            "notes": "",
                                        },
                                    ),
                                    (
                                        "external_header_blocks",
                                        {
                                            ...
                                            "nibbles": 6,
                                            "type": "binary",
                                            "description": "External header " "blocks",
    ...
    def __init__(self, path, net, sta, samprate, chanprfx, mxdbuf, st2cc, debug=False):
        ...
        self.n_channels = self.header_data["General Header"]["General Header Block #1"][
            "channel_sets_per_scan_type"
        ]["value"]
        self.pbuf = []
    ...
    def parse_lead_headers(self):
        """Parse the leading headers of a SEGD block."""
    ...
    def get_starttime(self):
        """Get the start time of a header block."""
    ...
    def __str__(self):
        """Return the string representation of a SEG-D Header."""
        ...
        for general_block in self.header_data:
            ...
            for header_block in self.header_data[general_block]:
                ...
                for field in self.header_data[general_block][header_block]:
                    ...
                    s = "%s\t\t%s: %s\n" % (
                        s,
                        self.header_data[general_block][header_block][field][
                            "description"
                        ],
                        self.header_data[general_block][header_block][field]["value"],
                    )
                    # getattr(self, field))
                s = "%s\n" % s
        return s
    ...
    def _read_header_block(self, block_schema):
        ...
        for field in block_schema:
            ...
            if block_schema[field]["type"] == "bcd":
                value = self._read_BCD(
                    block_schema[field]["start"], block_schema[field]["nibbles"]
                )
            elif block_schema[field]["type"] == "binary":
                value = self._read_binary(
                    block_schema[field]["start"], block_schema[field]["nibbles"]
                )
            elif block_schema[field]["type"] == "ieee":
                value = self._read_ieee(
                    block_schema[field]["start"], block_schema[field]["nibbles"]
                )
            else:
                self._problem("Data type %s not valid" % block_schema[field]["type"])
            return_dict[field] = {
                "value": value,
    ...
    def _read_BCD(self, start, nibbles):
        ...
        """
        data, ignore_first_nibble, ignore_last_nibble, n_bytes = self._get_raw_data(
            start, nibbles
        )
    ...
    def _read_binary(self, start, nibbles):
        ...
        if n_bytes == 1:
            ...
            if ignore_first_nibble:
                return_data = return_data & 0x0FFF
            if ignore_last_nibble:
                return_data = return_data >> 4
            return return_data
            ...
            if ignore_first_nibble:
                return_data = return_data & 0x0FFFFFFF
            if ignore_last_nibble:
                return_data = return_data >> 4
        elif n_bytes > 4:
            if n_bytes != 8:
    ...
    def read_trace_block(self):
        """Read a SEGD trace block."""
        header_data = OrderedDict([])
        header_data["20-byte Trace Header Block"] = self._read_header_block(
            self.schema["Trace Header"]["20-byte Trace Header Block"]
        )
        ...
        for n in range(
            ...
            ]["number_of_32_byte_trace_header_extensions"]["value"]
        ):
            header_block = "32-byte Trace Header Block #%d" % (n + 1)
            header_data[header_block] = self._read_header_block(
                self.schema["Trace Header"][header_block]
            )
            self.cursor_position += self.schema["Trace Header"][header_block][
                "block_length_in_bytes"
            ]
    ...
    def dump_trace_block(self):
        """Display contents of a SEGD trace block."""
    ...
    def fill_buffer(self, lsc):
        """Fill the read buffer. Collect stats."""
    ...
    def fill_prebuffer(self):
        ...
        self.cst = self._read_header_block(
            self.schema["Trace Header"]["32-byte Trace Header Block #1"]
        )["sensor_type_on_this_trace"]["value"]
        ...
        while True:
            ...
            # return cursor to beginning of current trace block.
            self.cursor_position += 20
            header_data = self._read_header_block(
                self.schema["Trace Header"]["32-byte Trace Header Block #1"]
            )
            self.cursor_position -= 20
    ...
    def write_slice(self, startyr, startjday, endyr, endjday, wdir):
        """Write a SEGD slice."""
        trseg = self.tr.slice(
            UTCDateTime(year=startyr, julday=startjday),
    ...
    def dump_buffer(self, wdir):
        """Dump (flush) buffers to disk."""
        self.write_buffer(wdir)
        self.write_stream(Stream([self.tr]), wdir)
        self.flush_buffer()

    def write_stream(self, st, wdir):
        """Write a stream of miniseed data to a file."""
        filename = "%s.%s.%d.%03d.%02d.%02d.%02d" % (
            st[0].stats["station"],
            st[0].stats["channel"],
